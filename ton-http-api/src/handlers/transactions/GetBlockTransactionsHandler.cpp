#include "GetBlockTransactionsHandler.h"

#include <boost/lexical_cast.hpp>

#include "converters/convert.hpp"

ton_http::handlers::GetBlockTransactionsHandler::GetBlockTransactionsHandler(
  const userver::components::ComponentConfig& config, const userver::components::ComponentContext& context
) :
    TonlibRequestHandler(config, context) {
}

ton_http::schemas::v2::BlockTransactionsRequest ton_http::handlers::GetBlockTransactionsHandler::
  ParseTonlibGetRequest(const HttpRequest& request, RequestContext&) const {
  schemas::v2::BlockTransactionsRequest req;
  try {
...
td::Status ton_http::handlers::GetBlockTransactionsHandler::ValidateRequest(
  const schemas::v2::BlockTransactionsRequest& request
) const {
  if (request.count <= 0) {
...
td::Result<ton_http::schemas::v2::BlockTransactions>
ton_http::handlers::GetBlockTransactionsHandler::HandleRequestTonlibThrow(
  ...
  TRY_RESULT(
    ...
    tonlib_component_.DoRequest(
      &core::TonlibWorker::getBlockTransactions,
      request.workchain,
      request.shard,
      request.seqno,
      request.count,
