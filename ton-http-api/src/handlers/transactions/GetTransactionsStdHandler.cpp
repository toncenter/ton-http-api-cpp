#include "GetTransactionsStdHandler.h"
...
ton_http::handlers::GetTransactionsStdHandler::GetTransactionsStdHandler(
...
td::Status ton_http::handlers::GetTransactionsStdHandler::ValidateRequest(
  ...
  if (request.limit.value_or(10) > 1000) {
    return td::Status::Error(422, "limit should be less or equal 1000");
  }
  bool has_lt = false;
  if (request.lt) {
    has_lt = true;
...
td::Result<ton_http::schemas::v2::TransactionsStd>
ton_http::handlers::GetTransactionsStdHandler::HandleRequestTonlibThrow(
  ...
  return converters::Convert<schemas::v2::TransactionsStd>(result);
}
