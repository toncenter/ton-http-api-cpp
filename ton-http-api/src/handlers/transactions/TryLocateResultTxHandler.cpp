#include "TryLocateResultTxHandler.h"

#include <boost/lexical_cast.hpp>

#include "converters/convert.hpp"

ton_http::handlers::TryLocateResultTxHandler::TryLocateResultTxHandler(
  const userver::components::ComponentConfig& config, const userver::components::ComponentContext& context
) :
    TonlibRequestHandler(config, context) {
}

ton_http::schemas::v2::TryLocateResultTxRequest ton_http::handlers::TryLocateResultTxHandler::ParseTonlibGetRequest(
  const HttpRequest& request, RequestContext&
) const {
  schemas::v2::TryLocateResultTxRequest req;
...
td::Status ton_http::handlers::TryLocateResultTxHandler::ValidateRequest(
  const schemas::v2::TryLocateResultTxRequest& request
  ) const {
...
td::Result<ton_http::schemas::v2::Transaction> ton_http::handlers::TryLocateResultTxHandler::HandleRequestTonlibThrow(
  schemas::v2::TryLocateResultTxRequest& request, multiclient::SessionPtr& session
) const {
  TRY_RESULT(
    result,
    tonlib_component_.DoRequest(
      &core::TonlibWorker::tryLocateTransactionByIncomingMessage,
