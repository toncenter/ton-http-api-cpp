...
ton_http::schemas::v2::ExtendedAddressInformationRequest ton_http::handlers::GetExtendedAddressInformationHandler::ParseTonlibGetRequest(
...
td::Result<ton_http::schemas::v2::ExtendedAddressInformation> ton_http::handlers::GetExtendedAddressInformationHandler::HandleRequestTonlibThrow(
    schemas::v2::ExtendedAddressInformationRequest& request, multiclient::SessionPtr& session
) const {
  auto result = tonlib_component_.DoRequest(&core::TonlibWorker::getExtendedAddressInformation, request.address.GetUnderlying(), request.seqno, session);
  if (result.is_error()) {
