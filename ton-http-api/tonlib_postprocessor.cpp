      ...
      td::overloaded(
          [&](tonlib_api::msg_dataRaw& data) {
            ...
            if (r_body.is_ok()) {
              ...
              if (r_body.is_ok()) {
                builder["message"] = td::base64_encode(r_body.move_as_ok());
              } else {
                  builder["message_decode_error"] = r_body.move_as_error_suffix("failed to decode msg body hex: ").to_string();
              }
            } else {
          ...
          [&](tonlib_api::msg_dataDecryptedText& data) { builder["message"] = data.text_; },
          [&](tonlib_api::msg_dataEncryptedText& data) { builder["message"] = data.text_; },
          [&](auto& x) { LOG(DEBUG) << "failed to decode type " << x.get_id(); }
      )
  );
  if (!success) {
